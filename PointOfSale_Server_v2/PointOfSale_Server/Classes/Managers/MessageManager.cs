...
namespace PointOfSale_Server.Classes.Managers
{
    ...
    //public delegate void OnKitchenStatusChanged(MessageBase RawMessage);
    ...
    public sealed class MessageManager
    {
        ...
        ////public static OnKitchenStatusChanged KitchenStatusChanged;
        ...
        private const int PORT_TWO = 9001;
        private static Socket ServerListenSocket = null;
        private static Socket ServerListenSocketTwo = null;
        ...
        private static void SetupMessageLookups()
        {
            ...
            //MessageLookup.Add("KITCHENSTATUS", new KitchenStatusMessage());
        }
        ...
        private static void  ListenForConnections()
        {
            while (true)
            {
                ...
                allDone.WaitOne();
                ServerListenSocketTwo.BeginAccept(
                    new AsyncCallback(AcceptCallback),
                    ServerListenSocketTwo);
            }
        }
        ...
        public static void StartServerSocket()
        {
            ...
            IPEndPoint localEndPoint = new IPEndPoint(LocalIPv4Address, PORT_NUMBER);
            IPEndPoint kitchenlocalEndPoint = new IPEndPoint(LocalIPv4Address, PORT_TWO);


            ServerListenSocket = new Socket(LocalIPv4Address.AddressFamily,
                         SocketType.Stream, ProtocolType.Tcp);
            ServerListenSocketTwo = new Socket(LocalIPv4Address.AddressFamily,
                         SocketType.Stream, ProtocolType.Tcp);
            ...
            try
            {
                ...
                ServerListenSocketTwo.Bind(kitchenlocalEndPoint);

                ServerListenSocket.Listen(10);
                ServerListenSocketTwo.Listen(10);

                ListenForMessages = new Thread(ListenForConnections);
                ListenForMessagesKitchen = new Thread(ListenForConnections);
        ...
        public static void HandleMessage(MessageBase Message)
        {
            switch (Message.GetMessageType())
            {
                ...
                case "PAYMENTACCEPTED":
                    ...
                    break;
                    // case "KITCHENSTATUS":
                    //  KitchenStatusChanged.Invoke(Message);
            }
        ...
        public static void CloseAllConnections()
        {
            ...
            ListenForMessages.Abort();
            try
            {
                ServerListenSocket.Shutdown(SocketShutdown.Both);
                ServerListenSocket.Close();
                ServerListenSocketTwo.Shutdown(SocketShutdown.Both);
                ServerListenSocketTwo.Close();
                
            }
            catch (Exception e)
            {
                ServerListenSocket.Dispose();
                ServerListenSocketTwo.Dispose();
            }
        }
    }
}
